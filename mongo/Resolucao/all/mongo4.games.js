...
games.init = function(server, security, apiBaseUri){
	server.get(apiBaseUri+'games', 
        security.passport.authenticate('bearer', { session: false }), getGames);
	server.get(apiBaseUri+'games/:id',
	      security.passport.authenticate('bearer', { session: false }), getGame);
	server.put(apiBaseUri+'games/:id',
	 	    security.passport.authenticate('bearer', { session: false }), updateGame);
	server.post(apiBaseUri+'games',
	 	    security.passport.authenticate('bearer', { session: false }), createGame);
	server.del(apiBaseUri+'games/:id',
	 	    security.passport.authenticate('bearer', { session: false }), deleteGame);
